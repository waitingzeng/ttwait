...
from custom.spider.models import BGSpiderUrls
...
class Index(ResponseHandler):
    def parse(self, **kwargs):
        ...
        for option in text.get_in_list(page.find('#cate_id').html(), '<option', '</option>'):
            ...
            yield Url(name, '/iframe-bg/c/%s/page/1' % value, self.response)


@route('/iframe-bg/c/(\d+)/page/(\d+)')
    ...
    def parse(self, cid, cur_page, **kwargs):
        ...
        for a in text.get_in_list(page.find('.page1').html(), '<a', '</a>'):
            if a.find('Next') != -1:
                ...
                yield Url('%s_%s' % (cid, cur_page), '/iframe-bg/c/%s/page/%s' % (cid, int(cur_page) + 1), self.response)
...
def main():
    start_urls = ['http://www.yescustomall.com/iframe-bg']
    pipeline = BGSpiderUrls.pipeline(start_urls)
