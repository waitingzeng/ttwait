    ...
    def commit(self):
        if self._db is not None:
            ...
            try:
                ...
                self._db.rollback()
                log.exception("Can not commit",e)
...
class Update(object):
    ...
    def __call__(self,*fields, **kwfields):
        if len(fields)<1 and len(kwfields) < 1:
...
class conds(object):
    ...
    def __getattr__(self,func_name):
        if not self._has_value:
            if str(array.__class__)=="database.Select":
                self.field_name="".join([func_name,"(t.",self.field_name,") as ",func_name,"_",self.field_name])
            else:
                self.field_name="".join([func_name,"(",self.field_name,") as ",func_name,"_",self.field_name])
            return self
        raise OperationalError,"Multiple Operate conditions"
