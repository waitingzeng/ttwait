...
class Mixin(object):
    ...
    def initmixin(self):
        ...
        if not self.__mixinname__:
            logging.warn('[Mixin] The class [%s] has not a mixinname defined' % self.__class__.__name__)
            return
        ...
        if __mixinset__.has_key(self.__mixinname__):
            logging.info('[Mixin] Mixinning class [%s]' % self.__class__.__name__)
    ...
    def callplugin(self, name, *args, **kwargs):
        ...
        for i in range(len(items)):
            #logging.info("[Mixin] Call plugin [%s]" % name)
            nice, f = items[i]
            f = import_func(f)
...
#                    logging.info('>>> %s (name=%s, args=%r, kwargs=%r)' % (self.__class__.__name__, name, args, kwargs))
#                    t = time.time()
                v = f(*args, **kwargs)
#                if debug.is_debug() and name not in ['on_update_ui', 'on_idle']:
#                    logging.info('--- end (time=%d)' % (time.time() - t))
            except SystemExit:
                raise
            except:
                raise
...
#           logging.info('[Mixin] Dealing module [%s]' % i)
...
def printMixin():
    logging.info("[Mixin] Printing mixin set...")
    for name, value in __mixinset__.items():
        ...
        for k in keys:
            t, f = mixins[k]
            if callable(f):
                logging.info("\t          |%s\t%s.%s" % (k, f.__module__, f.__name__))
            else:
                logging.info("\t          |%s %s" % (k, f))
        logging.info("\t   |----plugin")
        keys = plugins.keys()
        keys.sort()
        for k in keys:
            ...
            for nice, f in plugins[k]:
                if callable(f):
                    logging.info("\t\t          %d %s.%s" % (nice, f.__module__, f.__name__))
                else:
                    logging.info("\t\t          %d %s" % (nice, f))
