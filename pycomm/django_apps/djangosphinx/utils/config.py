...
def _get_database_engine(name):
    if name.endswith('postgresql'):
        return 'pgsql'
    if name.endswith('mysql'):
        return 'mysql'
    raise ValueError, "Only MySQL and PostgreSQL engines are supported by Sphinx. %s" % name
...
def get_default_params(db='default'):
    if db is None:
        db = 'default'
    db_conf = settings.DATABASES[db]
    return {
        'database_engine': _get_database_engine(db_conf['ENGINE']),
        'database_host': db_conf['HOST'],
        'database_password': db_conf['PASSWORD'],
        'database_user': db_conf['USER'],
        'database_port': db_conf['PORT'],
        'database_name': db_conf['NAME'],
        'log_file': '/var/log/sphinx/searchd.log',
        'data_path': '/var/data',
    }
DEFAULT_SPHINX_PARAMS = get_default_params()
...
def get_source_context(db, tables, index, valid_fields):
    params = get_default_params(db)
    params.update({
        'tables': tables,
        'source_name': index,
        'index_name': index,
...
def generate_source_for_model(model_class, index=None, sphinx_params={}):
    ...
    params = get_source_context(using, [table], index, valid_fields)
    params.update({
        'table_name': table,
...
def generate_source_for_models(model_classes, index=None, sphinx_params={}):
    ...
    if index is None:
        index = '_'.join(m._meta.db_table for m in model_classes)
    using = router.db_for_write(model_classes[0])
    for model in model_classes:
        if using != router.db_for_write(model):
            raise ValueError("only allow one db to generate")

    params = get_source_context(using, tables, index, valid_fields)
    params.update(sphinx_params)
