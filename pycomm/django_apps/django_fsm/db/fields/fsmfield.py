...
class FSMMeta(object):
    ...
    def add_transition(self, source, target, conditions=[], , transition_name='', transition_desc=''):
...
def transition(field=None, source='*', target=None, save=False, conditions=[]):
    ...
    def inner_transition(func):        
        if not hasattr(func, '_django_fsm'):
            ...
            @decorator
            def _change_state(func, instance, *args, **kwargs):
        ...
        if isinstance(source, (list, tuple)):
            for state in source:
                func._django_fsm.add_transition(state, target, conditions, transition_name, transition_desc)
        else:
            func._django_fsm.add_transition(source, target, conditions, transition_name, transition_desc)
