...
from .models import ModelStatusChange
...
def transition(field=None, source='*', target=None, save=True, conditions=[], verbose_name=None):
    """
    ...
    def inner_transition(func):        
        if not hasattr(func, '_django_fsm'):
            ...
            func.verbose_name = verbose_name or getattr(func, 'verbose_name', None) or func.__name__
            ...
            def _change_state(instance, *args, **kwargs):
                ...
                if save:
                    instance.save()
                    if field.choices:
                        status_map = dict(field.choices)
                    else:
                        status_map = {source_state : source_state, target : target}
...
class FSMField(models.Field):
    ...
    def contribute_to_class(self, cls, name):
        ...
        setattr(cls, 'fsm_field', self)
        setattr(cls, 'get_available_%s_transitions' % self.name, curry(get_available_FIELD_transitions, field=self))
        setattr(cls, 'get_available_transitions', curry(get_available_FIELD_transitions, field=self))

        func = lambda self, fieldname = name:getattr(self, fieldname)
        func.__name__ = self.name
        func.verbose_name = self.verbose_name
        setattr(cls, 'get_cur_status', func)

        if self.choices:
            func = lambda self, fieldname = name, choices=dict(self.choices) : choices[getattr(self, fieldname)]
            func.__name__ = self.name
            func.verbose_name = self.verbose_name

            setattr(cls, 'get_cur_status_display', func)
        else:
            setattr(cls, 'get_cur_status_display', func)
