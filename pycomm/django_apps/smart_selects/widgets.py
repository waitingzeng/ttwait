...
def get_final_choices_fk(self, value):
    final_choices = []
...
def get_final_choices_m2m(self, value):
    final_choices = []
    value = [x for x in value if x]
    try:
        item = self.queryset.filter(pk__in=value)[0]
    except IndexError:
        pass
    else:
...
class ChainedSelect(Select):
    ...
    def render(self, name, value, attrs=None, choices=()):
        ...
        final_choices = []
        if value:
            final_choices = get_final_choices_fk(self, value)
        ...
        if 'class' in final_attrs:
            ...
            final_attrs['class'] = 'chained'
        output = Select.render(self, name, value, final_attrs, choices=final_choices)
        output += js
...
class ChainedMultipleSelect(SelectMultiple):
    def __init__(self, app_name, model_name, chain_field, model_field, show_all, auto_choose, manager=None, *args, **kwargs):
        ...
        self.manager = manager
        self.limit_choices_to = kwargs.pop('limit_choices_to', {})
        SelectMultiple.__init__(self, *args, **kwargs)
